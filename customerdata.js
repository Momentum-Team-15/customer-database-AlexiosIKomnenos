...
function buildDirectory(customerArray) {
    for (let customer of customerArray) {
        ...
        dob.innerText = `Born: ${moment(customer.dob.date).format("MMMM Do YYYY")}`;
        customerDiv.appendChild(dob);
        customerContainer.appendChild(customerDiv);

        let registered = document.createElement("p"); 
        registered.classList.add("registered")
        registered.innerText = `A customer since: ${moment(customer.registered.date).format("MMMM Do YYYY")}`;
...
// moment(new Date(customer.dob.date)).format('MMMM Do YYYY, h:mm:ss a');
